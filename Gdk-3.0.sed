...
s/public func errorTrapPop/@available(*, deprecated) public func errorTrapPop/
s/public func errorTrapPopIgnored/@available(*, deprecated) public func errorTrapPopIgnored/
s/public func errorTrapPush/@available(*, deprecated) public func errorTrapPush/
...
s/let rv = gdk_window_get_device_position(cast.*mask))/let ptrPtr: UnsafeMutablePointer<GdkWindow> = cast(ptr)\n        let devicePtr: UnsafeMutablePointer<GdkDevice> = cast(device.ptr)\n        let xPtr: UnsafeMutablePointer<gint> = cast(x)\n        let yPtr: UnsafeMutablePointer<gint> = cast(y)\n        let maskPtr: UnsafeMutablePointer<GdkModifierType> = cast(mask)\n        let rv = gdk_window_get_device_position(ptrPtr, devicePtr, xPtr, yPtr, maskPtr)/
s/let rv = gdk_window_get_device_position_double(cast.*mask))/let ptrPtr: UnsafeMutablePointer<GdkWindow> = cast(ptr)\n        let devicePtr: UnsafeMutablePointer<GdkDevice> = cast(device.ptr)\n        let xPtr: UnsafeMutablePointer<gdouble> = cast(x)\n        let yPtr: UnsafeMutablePointer<gdouble> = cast(y)\n        let maskPtr: UnsafeMutablePointer<GdkModifierType> = cast(mask)\n        let rv = gdk_window_get_device_position_double(ptrPtr, devicePtr, xPtr, yPtr, maskPtr)/
